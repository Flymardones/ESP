...
NVS::NVS() {
    ...
    if (nvs_open("ESPStreamer", NVS_READWRITE, &nvsHandle) != ESP_OK) {
...
bool NVS::createNVSPartition() {
    if (nvs_flash_init_partition("ESPStreamer") != ESP_OK) {
...
void NVS::reset() {
    ...
    nvs_close(nvsHandle);
    nvs_flash_erase_partition("ESPStreamer");
    createNVSPartition();
    nvs_open("ESPStreamer", NVS_READWRITE, &nvsHandle);
}
